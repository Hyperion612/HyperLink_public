...
import { Save, User, Check, Crown } from 'lucide-react';
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="glass rounded-2xl p-6 space-y-6">
            ...
            <div className="border-t border-white/5 pt-6 space-y-4">
              ...
              <div>
                <label className="block text-sm text-gray-400 mb-1.5">Тариф</label>
                <div className="flex items-center gap-3">
                  <span className="px-3 py-1.5 rounded-lg text-sm font-medium" style={{ background: user.plan === 'pro' ? 'rgba(59,130,246,0.1)' : 'rgba(255,255,255,0.05)', color: user.plan === 'pro' ? '#60A5FA' : '#9CA3AF', border: `1px solid ${user.plan === 'pro' ? 'rgba(59,130,246,0.2)' : 'rgba(255,255,255,0.1)'}` }}>
                    {user.plan === 'pro' ? 'Pro' : 'Free'}
                  </span>
                  {user.plan !== 'pro' && (
                    <button onClick={() => navigate('/pricing')} className="text-sm text-blue-400 hover:text-blue-300 transition-colors">
                      Обновить →
                    </button>
                  )}
                </div>
